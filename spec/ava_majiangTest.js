...
test("清一色听牌", function(t) {
  let str = "b1 b2 b2 b3 b3 b4 b4 b5 b5 b5 b6 b7 b7";
  t.deepEqual(Majiang.HuWhatPai(str).map(item=>item.hupai_zhang), ["b5", "b7"]);
});
test("清一色听7张", function(t) {
  let str = "b1 b2 b3 b4 b5 b6 b7 b8 b8 b8 b9 b9 b9";
  t.deepEqual(Majiang.HuWhatPai(str).map(item=>item.hupai_zhang), [
    "b1",
    "b3",
    "b4",
    "b6",
...
test("双将倒", function(t) {
  ...
  t.deepEqual(Majiang.HuWhatPai(str).map(item=>item.hupai_zhang), ["di", "zh"]);
});
test("单钓将", function(t) {
  let str = "b1 b1 b2 b2 b3 b3 b4 b5 b6 b7 b7 b7 b8";
  t.deepEqual(Majiang.HuWhatPai(str).map(item=>item.hupai_zhang), ["b3", "b6", "b8", "b9"]);
});
...
test("碰碰胡听牌", function(t) {
  ...
  t.deepEqual(Majiang.HuWhatPai(str).map(item=>item.hupai_zhang), ["t2", "t3"]);
});
...
test("龙七对听牌", function(t) {
  let str = "b1 b1 b2 b2 fa fa fa fa t1 t1 t4 t4 t9";
  t.deepEqual(Majiang.HuWhatPai(str).map(item=>item.hupai_zhang), ["t9"]);
});
