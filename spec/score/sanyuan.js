...
test.beforeEach(t => {
  player1 = new Player({
    group_shou_pai: {
      ...
      peng: [],
      selfPeng: [],
      shouPai: getArr("b1 b1 b1 b2 b3 di di di fa fa zh zh zh")
    },
    socket: null,
    username: "jack1",
    user_id: 10001
  ...
  player1.is_hu = true;
  player1.hupai_zhang = "b1";
  player1.hupai_data = {
    hupai_dict: { "b1": [config.HuisPihu, config.HuisXiaoShanYuan] }
  };
...
test("大三元放炮", function(t) {
  ...
  player1.hupai_data = {
    hupai_dict: { "b1": [config.HuisPihu, config.HuisDaShanYuan] }
  };
  ScoreManager.cal_oneju_score([player1, player2, player3]);
  console.log(
    `各自分数： 
