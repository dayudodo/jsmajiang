...
test("没有亮，selfPeng能扛", function(t) {
  let group_shou_pai = {
    // anGang: ["zh"],
    anGang: [],
    anGangCount: 0,
...
test("group 杠牌在亮牌中，亮牌后不能扛", function(t) {
  ...
  let flatShou = NMajiangAlgo.flat_shou_pai(group_shou_pai);
  t.is(NMajiangAlgo._canGang(flatShou, to_number("b9")), true);
  t.is(NMajiangAlgo.canGang(group_shou_pai, to_number("b9"), true), false);
});
test("group 亮牌不能碰,但是可以杠", function(t) {
  ...
  let isLiang = true,
    selfMo = true;
  let canPengShou = NMajiangAlgo.canPeng(
    group_shou_pai.shouPai,
    to_number("t3"),
    isLiang
  );
  t.is(canPengShou, false); //不能再碰了
