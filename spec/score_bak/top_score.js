...
test.beforeEach(t => {
  ...
  player1.is_hu = true;
  player1.hupai_zhang = "b5";
  player1.hupai_data = {
    hupai_dict: { "b5": [config.HuisPihu, config.HuisKaWuXing] }
  };
...
test("清一色卡五星亮倒自摸", function(t) {
  ...
  player1.hupai_data = {
    hupai_dict: { "b5": [config.HuisPihu, config.HuisKaWuXing, config.HuisYise, config.HuisLiangDao] }
  };
