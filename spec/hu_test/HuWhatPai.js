...
test("group后碰碰胡听牌只胡一", function (t) {
  ...
  t.deepEqual(MajiangAlgo.HuWhatGroupPai(group_shoupai).all_hupai_typesCode, [config.HuisPengpeng, config.HuisPihu])
...
test("龙七对听牌", function (t) {
  ...
  t.deepEqual(MajiangAlgo.HuWhatPai(str).all_hupai_typesCode,
    [config.HuisQidui, config.HuisNongQiDui, config.HuisPihu])
...
test("group仅屁胡 zh", function (t) {
  ...
  t.deepEqual(MajiangAlgo.HuWhatGroupPai(group_shoupai).hupai_dict, { 'zh': [config.HuisPihu] })
})
...
test("大小三元 b1 b4 fa", function (t) {
  ...
  t.deepEqual(MajiangAlgo.HuWhatPai(str).all_hupai_typesCode,
    [config.HuisXiaoShanYuan, config.HuisDaShanYuan, config.HuisPihu])
