...
class NMajiangAlgo {
     ...
     * @param mo_pai 玩家摸到的牌
     */
    static canGangPais(group_shoupai, mo_pai) {
        let output = [];
        //peng里面是否包含shouPai中的一张，自己摸的，还要判断下有没有mo_pai!
        output = output.concat(group_shoupai.peng.filter(pai => group_shoupai.shouPai.includes(pai)));
        //自己隐藏的selfPeng里面是否包含shouPai中的自摸牌？
        output = output.concat(group_shoupai.selfPeng.filter(pai => group_shoupai.shouPai.includes(pai)));
        if (!_.isEmpty(mo_pai)) {
